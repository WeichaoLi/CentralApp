...
@property (strong, nonatomic) CBPeripheral *peripheral;//连接的外围设备
...
- (IBAction)sendData:(id)sender {
    if (self.peripheral.state == CBPeripheralStateConnected) {
        //中心设备发送数据
        NSString *str = @"李伟超发的数据";
        [self.peripheral writeValue:[str dataUsingEncoding:NSUTF8StringEncoding] forCharacteristic:self.characteristic type:CBCharacteristicWriteWithResponse];
    }
}
...
-(void)centralManager:(CBCentralManager *)central didDiscoverPeripheral:(CBPeripheral *)peripheral advertisementData:(NSDictionary *)advertisementData RSSI:(NSNumber *)RSSI{
    ...
    [self writeToLog:[NSString stringWithFormat:@"发现外围设备...%@", peripheral.name]];
...
-(void)centralManager:(CBCentralManager *)central didConnectPeripheral:(CBPeripheral *)peripheral{
    ...
    //外围设备开始寻找服务
//    [peripheral discoverServices:@[[CBUUID UUIDWithString:kServiceUUID]]]; //指定服务
    [peripheral discoverServices:nil];  //不指定服务
}
//连接外围设备失败
